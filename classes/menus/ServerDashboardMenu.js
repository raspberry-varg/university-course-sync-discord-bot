...
class ServerDashboardMenu extends InteractiveMenu {
    ...
    constructor( interaction, databaseInfo ) {
        ...
        this.addPage({
            ...
            fields: [
                {
                    name: '⚙️ **Config**\n**= = = = = = =**\n\u200b\n♾️ Any Class Allowed: ' + ( databaseInfo.any ? '৹ Yes' : '✕ No' ),
                    value: '\u200b\n**= = = = = = =**',
                    inline: false,
                },
                {
                    ...
                    if ( this.changesMade ) {
                        ...
                        await this.interaction.guild.members.fetch().then( async m => {
                            ...
                            for ( const [ k, member ] of m ) {
                                ...
                                if ( userData )
                                    await refreshUserClasses( this.interaction, userData, this.dbServer );
                            }
    ...
    async subjectPage() {
        ...
        MCollector.on('collect', async c => {
            ...
            let validated = validateSubjectInput( c.content.toString(), this.interaction.client.courses );
            if ( !validated ) {
    ...
    async courseWhitelistPage() {
        ...
        MCollector.on('collect', async c => {
            ...
            console.log(c.content)
            let validated = validate( c.content.toString(), this.interaction.client.courses );
            if ( !validated ) {
